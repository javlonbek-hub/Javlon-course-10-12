...
//         }' data-has-car='${person.hasCar}' data-position='${index + 1
...
dataPeople.forEach((dataPerson) => {
  dataPerson.addEventListener("click", () => {
    ...
    const status = dataPerson.dataset.status; // Преобразуем строку в булево значение
    const car = dataPerson.dataset.hasCar; // Преобразуем строку в булево значение
    ...
    moreInfo.insertAdjacentHTML("beforeend",
      ...
      <p>Возраст: ${age}</p>
      <p>Семейное положение: ${status ? "Женат/Замужем" : "Xолост/Незамужем"
      }</p>
